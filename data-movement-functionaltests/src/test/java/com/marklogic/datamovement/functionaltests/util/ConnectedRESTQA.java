...
public abstract class ConnectedRESTQA {
	...
	public static void createDB(String dbName)	{
		try {	
			...
			HttpPost post = new HttpPost("http://"+host+":8002"+ "/manage/v2/databases?format=json");
...
//			System.out.println( getBootStrapHostFromML());
	...
	public static void changeProperty(Map<String, String> properties, String endpoint)	{
		try{
			...
			HttpPut post = new HttpPut("http://"+host+":8002"+ endpoint); 
			post.addHeader("Content-type", "application/json");
	...
	public static void createUserRolesWithPrevilages(String roleName, String... privNames ){
		try{
			...
			client.getCredentialsProvider().setCredentials(
					new AuthScope(host, 8002),
					new UsernamePasswordCredentials("admin", "admin"));
			HttpGet getrequest = new HttpGet("http://"+host+":8002/manage/v2/roles/"+roleName);
			HttpResponse resp = client.execute(getrequest);
			...
			if( resp.getStatusLine().getStatusCode() == 200)
			{
				...
				HttpPost post = new HttpPost("http://"+host+":8002"+ "/manage/v2/roles?format=json");
				post.addHeader("Content-type", "application/json");
	...
	public static void createRESTUser(String usrName, String pass, String... roleNames ){
		try{
			...
			client.getCredentialsProvider().setCredentials(
					new AuthScope(host, 8002),
					new UsernamePasswordCredentials("admin", "admin"));
			HttpGet getrequest = new HttpGet("http://"+host+":8002"+ "/manage/v2/users/"+usrName);
			HttpResponse resp = client.execute(getrequest);
			...
			if( resp.getStatusLine().getStatusCode() == 200)
			{
				...
				System.out.println(mainNode.toString());
				HttpPost post = new HttpPost("http://"+host+":8002"+ "/manage/v2/users?format=json");
				post.addHeader("Content-type", "application/json");
				post.setEntity(new StringEntity(mainNode.toString()));
...
"adfadsfads"
	...
	public static void deleteDB(String dbName){
		try{
			...
			HttpDelete delete = new HttpDelete("http://"+host+":8002/manage/v2/databases/"+dbName);
			client.execute(delete);
	...
	public static void waitForServerRestart()
	{
		try{
			...
			while(count <20){
				DefaultHttpClient client = new DefaultHttpClient();
				client.getCredentialsProvider().setCredentials(
						new AuthScope(host, 8001),
						new UsernamePasswordCredentials("admin", "admin"));

				count++;
				try{
					HttpGet getrequest = new HttpGet("http://"+host+":8001/admin/v1/timestamp");
...
//							System.out.println("Adding child array include node" + jnode.toString());
		...
		String namespaceStart, String localnameStart, String namespaceEnd, String localnameEnd) throws Exception
	{
		...
		HttpPost post = new HttpPost("http://"+host+":8002/manage/v2/databases/"+ dbName + "/temporal/axes?format=json");
			...
			throws Exception
	{
		...
		HttpPost post = new HttpPost("http://"+host+":8002/manage/v2/databases/"+ dbName + "/temporal/collections?format=json");
			...
			throws Exception
	{
		...
		HttpPut put = new HttpPut("http://"+host+":8002/manage/v2/databases/"+ dbName + "/temporal/collections/lsqt/properties?collection=" + collectionName);
